  ...
  */
  ...
  MCP23017.prototype._setGpioAPinValue = function (pin, value, callback) {
    ...
    if (value === 0) {
      if ((this.gpioAState & pinHexMask) === pinHexMask) {
        ...
        this._send2(WRITE_GPIOA_ADDR, [this.gpioAState], callback);
      }
    }
    if (value === 1) {
      if ((this.gpioAState & pinHexMask) !== pinHexMask) {
  ...
  MCP23017.prototype._setGpioBPinValue = function (pin, value, callback) {
    ...
    if (value === 0) {
      if ((this.gpioBState & pinHexMask) === pinHexMask) {
        ...
        this._send2(WRITE_GPIOB_ADDR, [this.gpioBState], callback);
      }
    }
    if (value === 1) {
      if ((this.gpioBState & pinHexMask) !== pinHexMask) {
  ...
  MCP23017.prototype.digitalWrite = function (pin, value, callback) {
    ...
    if (isNaN(pin)) {
      ...
      //Port A
      this._setGpioAPinValue(pin, value, function (err, registerValue) {
        if (err) {
          console.error(err);
          callback(pin, err, null);
        } else {
          callback(pin, null, false);
        }
      });
    } else {
      //Port B
      pin -= 8;
      this._setGpioBPinValue(pin, value, function (err, registerValue) {
        if (err) {
          console.error(err);
          callback(pin, err, null);
        } else {
  ...
  MCP23017.prototype._send2 = function (cmd, values, callback) {
    console.log('send2: ' + cmd + 'values: ' + values)
    var buff = Buffer.from(values);

    this.wire.writeI2cBlock(this.address,cmd,buff.length,buff,function (err, bytesRead, buffer) {
      if (err) {
        console.error(err);
        console.log(err);
        callback(err, null);
